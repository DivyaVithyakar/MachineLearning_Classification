...
# Multinomial Naïve Bayes requires **non-negative integer data**, so we convert everything to positive
x_train = x_train.abs()
x_test = x_test.abs()
...
grid = GridSearchCV(estimator=MultinomialNB(), param_grid=param_grid, cv=5, refit=True, scoring='accuracy', n_jobs=-1, verbose=1)
